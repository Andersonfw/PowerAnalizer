...
// Private functions -----------------------------------------------------------
	...
	// *****************************************************************************
	/// @brief		Fun��o que gera um c�digo CRC16
	/// @fn			u16 CRC16(u8 *pu8Data, s8 s8DataLength, u16 u16InitCRC)
	/// @param[in]	pu8Data			@brief Ponteiro para o buffer de dados
	/// @param[in]	sc8DataLength	@brief Tamanho do buffer de dados
	/// @param[in]	u16InitCRC		@brief Valor inicial do CRC
	/// @retval		CRC16
	// *****************************************************************************
	uint16_t CRC16(uint8_t *pu8Data, int8_t  s8DataLength, uint16_t u16InitCRC)
	{
		...
		u16Crc = u16InitCRC;
		for (i = 0 ; i < s8DataLength; i++)
			u16Crc = (u16Crc << 8) ^ CRC16Table[(u16Crc >> 8) ^ *pu8Data++];
		return u16Crc;
	}

	uint8_t crc8x_fast(uint8_t *pu8Data,int8_t  s8DataLength, uint8_t u8InitCRC)
	{
		uint8_t *data = pu8Data;
	    if (data == NULL)
	        return 0xff;
	    u8InitCRC &= 0XFF;
	    while (s8DataLength--)
	    	u8InitCRC = CRC8Table[u8InitCRC ^ *data++];
	    return u8InitCRC;
	}
	...
	uint8_t crc8x_simple(uint8_t *pu8Data,int8_t  s8DataLength, uint8_t u8InitCRC)
	{
	    ...
	    while (s8DataLength--) {
	        ...
	        for (unsigned k = 0; k < 8; k++)
	        	u8InitCRC = u8InitCRC & 0x80 ? (u8InitCRC << 1) ^ 0x31 : u8InitCRC << 1;
	    }
	    u8InitCRC &= 0xFF;
	    return u8InitCRC;
	}
	...
	/// @retval		CRC16
	// *****************************************************************************
	uint16_t CRC16(uint8_t *pu8Ptr, int8_t s8DataLength, uint16_t u16InitCRC)
	{
		uint8_t i;
		uint16_t u16Crc;

		u16Crc = u16InitCRC;
		while (--s8DataLength >= 0)
		{
			u16Crc = u16Crc ^ ((uint16_t)*pu8Ptr++ << 8);
			for(i = 0; i < 8; i++)
			{
				if (u16Crc & 0x8000)
